...
function finalize(root=path.resolve(__dirname,'..'),env=process.env){
 ...
 report=report.replace('The build environment passed unit/HTTP tests but could not run Chromium or a genuine model-driven browser session. Those gates remain outstanding, documented in `evidence/STATUS.md`.','GitHub Actions verified the real browser and genuine model-driven discovery/replay path; `evidence/verification.json` records counts, source commit, workflow and artifact lineage.');
 report=report.replace('Separate Chromium tests exercise the actual UI and are not represented as having passed here. Genuine discovery plus changed-input replay must be collected before submission.','Separate Chromium tests passed against the actual UI. The repository also includes genuine model discovery and changed-input deterministic replay evidence.');
 report=report.replace('Browser verification and genuine discovery evidence are different: they are incomplete required gates caused by the available environment, not optional scope cuts. The included collector and browser suite make those gates reproducible once Chromium/network/model access is available. Publication and submission also remain outstanding.','Browser verification and genuine discovery evidence were completed on GitHub Actions and are committed with their source lineage. The repository is public. Sending the submission email remains the applicant’s action.');
